...
export default function TotalGrowthBarChart({ isLoading, totalEarnings = 0, monthlyEarnings = [] }) {
  ...
  // Build dynamic monthly series from real monthly earnings
  const hasRealData = monthlyEarnings && monthlyEarnings.length > 0 && monthlyEarnings.some(v => v > 0);
  const baseData = hasRealData ? monthlyEarnings : [35, 125, 35, 35, 35, 80, 35, 20, 35, 45, 15, 75];

  const dynamicSeries = [
    { name: 'Investment', data: baseData.map(v => Math.round(v * 0.55)) },
    { name: 'Loss / Discount', data: baseData.map(v => Math.round(v * 0.08)) },
    { name: 'Profit', data: baseData.map(v => Math.round(v * 0.37)) },
    { name: 'Maintenance', data: baseData.map(v => Math.round(v * 0.05)) }
  ];

  const formattedTotal = Number(totalEarnings || 0).toLocaleString('en-US', {
    style: 'currency',
    currency: 'USD'
  });
      ...
      {isLoading ? (
        ...
        <MainCard>
          <Stack sx={{ gap: gridSpacing }}>
            ...
            >
              <Chart options={chartOptions} series={dynamicSeries} type="bar" height={480} />
            </Box>
          </Stack>
        </MainCard>
      )}
